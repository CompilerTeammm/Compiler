...
bool LoopRotaing::RotateLoop(Loop *loop, bool Succ)
{
    ...
    else
    {
          ...
          new_inst->GetOperand(1)->isConst())
      {
        simplify = ConstantFold::ConstFoldBinaryOps(
            ...
            dynamic_cast<BinaryInst *>(new_inst)->GetOp(),
  ...
  DominantTree::TreeNode *exitNode = m_dom.getNode(prehead);
  m_dom.getNode(prehead)->predNodes.push_front(exitNode);
...
void LoopRotaing::SimplifyBlocks(BasicBlock *Header, LoopInfo *loop)
{
...
void LoopRotaing::PreserveLcssa(BasicBlock *new_exit, BasicBlock *old_exit,
                                BasicBlock *pred)
{
...
bool LoopRotaing::TryRotate(LoopInfo *loop)
{
  bool Legal = false;
  auto latch = loopAnlasis->GetLatch(loop);
