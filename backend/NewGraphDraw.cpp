...
void GraphColor::SpillNodeInMir()
{
  ...
  for (const auto mbb : topu)
  {
    ...
    {
      auto mir = *mir_begin;
      if (mir->GetOpcode() == RISCVMIR::call || mir->GetOpcode() == RISCVMIR::ret || mir->GetOpcode() == RISCVMIR::_j)
      {
        ++mir_begin;
        continue; // 跳过特定指令
      }
