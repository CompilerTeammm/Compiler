...
void Legalize::run()
{
            ...
            for (auto it = block->begin(); it != block->end();)
            {
                ...
                case RISCVMIR::LoadImmReg:
                {
                    ...
                    if (isfloatconst)
                    {
                        ...
                        it = List<RISCVBasicBlock, RISCVMIR>::iterator(inst);
                        it.InsertBefore(liinst);
                        it.InsertBefore(fmvwx);
                        delete inst;
                        it = List<RISCVBasicBlock, RISCVMIR>::iterator(fmvwx);
                        break;
                    }
                    else
                    {
    ...
    }
    {
        ...
        while (time < legalizetime)
        {
            ...
            for (List<RISCVBasicBlock, RISCVMIR>::iterator it = func->GetExit()->begin(); it != func->GetExit()->end(); ++it)
            {
                LegalizePass(it);
            }
            time++;
...
void Legalize::LegalizePass(List<RISCVBasicBlock, RISCVMIR>::iterator it)
{
...
void Legalize::LegalizePass_before(List<RISCVBasicBlock, RISCVMIR>::iterator it)
{
    using PhyReg = PhyRegister::PhyReg;
    using ISA = RISCVMIR::RISCVISA;
    RISCVMIR *inst = *it;
...
void Legalize::LegalizePass_after(List<RISCVBasicBlock, RISCVMIR>::iterator it)
{
    using PhyReg = PhyRegister::PhyReg;
    using ISA = RISCVMIR::RISCVISA;
    RISCVMIR *inst = *it;
...
void Legalize::StackAndFrameLegalize(int i, List<RISCVBasicBlock, RISCVMIR>::iterator &it)
{
    RISCVMIR *inst = *it;
...
void Legalize::OffsetLegalize(int i, List<RISCVBasicBlock, RISCVMIR>::iterator &it)
{
    RISCVMIR *inst = *it;
    StackRegister *sreg = nullptr;
    if (sreg = dynamic_cast<StackRegister *>(inst->GetOperand(i)))
...
void Legalize::zeroLegalize(int i, List<RISCVBasicBlock, RISCVMIR>::iterator &it)
{
    RISCVMIR *inst = *it;
...
void Legalize::branchLegalize(int i, List<RISCVBasicBlock, RISCVMIR>::iterator &it)
{
    RISCVMIR *inst = *it;
    RISCVMIR *li = new RISCVMIR(RISCVMIR::li);
    PhyRegister *t0 = PhyRegister::GetPhyReg(PhyRegister::t0);
...
void Legalize::noImminstLegalize(int i, List<RISCVBasicBlock, RISCVMIR>::iterator &it)
{
    RISCVMIR *inst = *it;
...
void Legalize::constintLegalize(int i, List<RISCVBasicBlock, RISCVMIR>::iterator &it)
{
    RISCVMIR *inst = *it;
    Imm *constdata = dynamic_cast<Imm *>(inst->GetOperand(i));
    PhyRegister *t0 = PhyRegister::GetPhyReg(PhyRegister::t1);
